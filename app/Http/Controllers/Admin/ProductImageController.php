...
class ProductImageController extends Controller
{
    ...
    public function destroy(Request $request){
        ...
        alert()->success('تصویر مورد نظر حذف شد','موفق');
    ...
    public function setPrimary(Request $request,Product $product){
       ...
       $productImage=ProductImage::findOrFail($request->image_id);
        ...
        alert()->success('تصویر مورد نظر با موفقیت ویرایش شد', 'موفق');
        return redirect()->back();
    }
    ...
    public function add(Request $request,Product $product){
        ...
        if($request->primary_image == null && $request->images == null) {
            return redirect() - back()->withErrors([
                    'msg' => ' تصویر یا تصاویر محصول الزامی است'
                ]);
        }
            try{
                DB::beginTransaction();
                ...
                foreach ($request->images as $image) {
                    $fileNameImage = generateFileName($image->getClientOriginalName());
                    $request->images->move(public_path(env('PRODUCT_IMAGES_UPLOAD_PATH')), $fileNameImage);
        ...
        alert()->success('ویرایش تصویر با موفقیت انجام شد','موفق');
        return redirect()->back();
    }
}
