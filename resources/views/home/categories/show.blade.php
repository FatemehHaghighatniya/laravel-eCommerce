                        ...
                        <div class="sidebar-widget">
                            ...
                            <div class="pro-sidebar-search mb-50 mt-25">
                                <form class="pro-sidebar-search-form" action="#">
                                    <input  id="search-input" type="text" placeholder="... جستجو " value="{{request()->has('search') ? request()->search : '' }}"
                                    >
...
{{--                            <h4 class="pro-sidebar-title">{{ $variation->name }} </h4>--}}
    ...
    <form id="filter-form">
        <input type="hidden" name="variation" id="filter-variation">
        @foreach ($attributes as $attribute)
            <input id="filter-attribute-{{ $attribute->id }}" type="hidden" name="attribute[{{ $attribute->id }}]">
        @endforeach
        <input type="hidden" name="sortBy" id="filter-sort-by">
        <input type="hidden" name="search" id="filter-search">
    </form>
    <!-- Modal -->
    @foreach($products as $product)
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                       ...
                       <form action="{{ route('home.cart.add') }}" method="POST">
                           ...
                           <input type="hidden" name="variation" id="filter-variation" value="{{Json_encode($variation->only(['id','quantity','is_sale','sale_price','price']))}}">

                               @csrf
                                    @if($product->quantity_check)
                                        @php
                                            if($product->sale_check){
                                                  $variationSelected=$product->sale_check;
                                                       }else{
                                                   $variationSelected=$product->price_check;
                                                       }
                                        @endphp
                                                ...
                                                <div class="pro-details-size-content">
                                                    <select class="form-control variation-select">
                                                        @foreach($product->variations()->where('quantity','>',0)->get() as $variation)
                                                            <option value="{{Json_encode($variation->only(['id','quantity','is_sale','sale_price','price']))}} {{$variationSelected->id == $variation->id ? 'selected' : '' }}">
                                                                {{$variation->value}}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                        ...
                                        <div class="pro-details-quality">
                                            <div class="cart-plus-minus">
                                                <input class="cart-plus-minus-box" type="text" name="qtybutton" value="1" data-max="100"/>
                                            </div>
                                    ...
                                    <div class="pro-details-meta">
                                        <span>دسته بندی :</span>
                                        <ul>
                                            <li><a href="#">{{$product->category->name}}-{{$product->category->parent->name}}</a></li>
                                        </ul>
                                    </div>
                                    <div class="pro-details-meta">
                                        تگ ها  :
                            ...
                            <div class="col-md-5 col-sm-12 col-xs-12">
                                ...
                                <div class="quickview-wrap mt-15">
                                    <div class="quickview-slide-active owl-carousel nav nav-style-2" role="tablist">
                                        <a class="active" data-toggle="tab" href="#pro-primary-{{$product->id}}"><img src="{{asset(env('PRODUCT_IMAGES_UPLOAD_PATH').$product->primary_image)}}" alt="" /></a>
                                        @foreach($product->images as $image)
                                            <a data-toggle="tab" href="#pro-{{$image->id}}"><img src="{{asset(env('PRODUCT_IMAGES_UPLOAD_PATH').$image->image)}}"
                                                                                                 alt="" /></a>
                                        @endforeach
